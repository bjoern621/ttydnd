...
using whichever of `sh`, `bash`, `zsh`, `dash` and `ash` are on `PATH`.
...
The kitty backend replaces `Window.on_drop` and drives the progress marker through kitty's own classes,
so a kitty release can move either.
